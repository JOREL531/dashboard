...
# CONFIG GÉNÉRALE
# =========================================================
st.set_page_config(page_title="Cancer Prediction Dataset", page_icon=None, layout="wide")

# =========================================================
# DONNÉES + FEATURES DE BASE
# =========================================================
...
page = st.sidebar.radio(
    "Aller à",
    options=["Dashboard", "Modèle de prédiction", "Patients à haut risque"],
    index=0,
)
...
def page_dashboard():
    ...
    # Filtres spécifiques au dashboard (dans la barre latérale)
    ...
    genetic_filter = st.sidebar.multiselect(
        "Risque génétique", options=["Faible", "Moyen", "Élevé"], default=["Faible", "Moyen", "Élevé"]
    )
    ...
    with col1:
        st.info("EFFECTIF")
        st.metric(label="Nombre de patients suivis", value=len(df_filtered))
    with col2:
        st.info("DIAGNOSTIC")
        st.metric(label="% Cancer", value=f"{100 * df_filtered.Diagnosis.mean():.1f}%")
    with col3:
        st.info("IMC MOYEN")
        st.metric(label="Indice de masse corporelle", value=f"{df_filtered.BMI.mean():.1f}")
    with col4:
        st.info("ÂGE")
    ...
    heatmap_fig = px.imshow(
        ...
        x=ct_cancer["classe_age"],
        y=["Cancer"],
        color_continuous_scale="Reds",
        text_auto=True,
    )
    heatmap_fig.update_traces(textfont=dict(size=18))
    ...
    barplot_groupe_fig = px.bar(
        ...
        barmode="group",
        text="Count",
        color_discrete_map={"Pas de cancer": "#8A91F5", "Cancer": "#EF553B"},
    )
    barplot_groupe_fig.update_traces(textfont=dict(size=18))
    ...
    moyennes = (
        df_filtered.groupby("Diagnosis")[["PhysicalActivity", "AlcoholIntake"]].mean().reset_index()
    )
    ...
    smokers = df_filtered[df_filtered["Smoking"] == 1]
    ...
    nonsmokers = df_filtered[df_filtered["Smoking"] == 0]
...
def page_modele():
    ...
    # On expose pour réutilisation éventuelle
    globals()["model"] = model
    globals()["X"] = X
    ...
    with st.expander("Performances du modèle (ROC, Gini, matrice de confusion)"):
        ...
        roc_fig = px.line(roc_df, x="FPR", y="TPR", title=f"Courbe ROC (AUC = {auc:.3f}, Gini = {gini:.3f})")
        ...
        cm_df = pd.DataFrame(
            cm,
            index=["Vrai : 0 (Pas de cancer)", "Vrai : 1 (Cancer)"],
        ...
        with st.expander("Contribution des facteurs (importance des variables)"):
            ...
            coef_df = pd.DataFrame(
                {"Variable": variables, "Coefficient": coef, "Contribution": np.abs(coef)}
            ).sort_values(by="Contribution", ascending=True)

            st.markdown(
                """
                Interprétation des coefficients :
                - Valeur positive → augmente la probabilité d'avoir un cancer
                - Valeur négative → réduit la probabilité estimée
                """
            )
            ...
            fig_coef = px.bar(
                ...
                orientation="h",
                color="Coefficient",
                color_continuous_scale=["#4B56EB", "#E63718"],
                text="Coefficient",
                title="Contribution des facteurs au risque estimé de cancer",
            )
            fig_coef.update_traces(
                texttemplate="%{text:.3f}", textposition="outside", textfont=dict(size=14, color="white")
            )
...
# =========================================================
# PAGE 3 — PATIENTS À HAUT RISQUE
# IMPORTANT : Bloc repris tel quel (logique inchangée)
# =========================================================
def page_patients_risque():
    st.title("Patients à haut risque")
    ...
    # --- Colonne de probabilités ---
    try:
        # si le modèle global existe déjà (ex : tu as ouvert la page "Modèle de prédiction")
        df[proba_col] = model.predict_proba(X)[:, 1]

    except Exception:
        # fallback : on entraîne le même modèle ici pour pouvoir calculer les probas
        feature_cols = ["Age", "BMI", "Smoking", "GeneticRisk", "PhysicalActivity", "AlcoholIntake"]
        X_local = df[feature_cols]
        y_local = df["Diagnosis"]
        ...
        model_local = LogisticRegression(max_iter=1000)
        model_local.fit(X_tr, y_tr)

        # on expose aussi pour les usages ultérieurs dans la session
        globals()["model"] = model_local
        globals()["X"] = X_local

        df[proba_col] = model_local.predict_proba(X_local)[:, 1]
